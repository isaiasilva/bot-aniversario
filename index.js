...
console.log(centralizarTexto('#     BATISTA BIBLICA ISRAEL            #'));
console.log(centralizarTexto('#                                       #'));
console.log(centralizarTexto('#########################################'));
...
const buscarMensagensCasamentos = async () => {
    try {
        // Requisição GET para a API de noivados
        const response = await axios.get('https://crm.biblicaisrael.com.br/api/birthdays_married.php');

        if (response.status === 200) {
            const noivados = response.data;

            for (const noivado of noivados) {
                const nome = noivado.nome;
                const numeroTelefone = noivado.numeroTelefone;
                const mensagem = noivado.textMessage;

                //se nao tiver nenhum noivado no dia, nao envia mensagem
                if (nome == null) {
                    console.log(chalk.red('[!] Nenhum aniversariante de casamento no dia de hoje (' + dataAtual + ')!'));
                    break;
                } else {

                    client.sendMessage(`${numeroTelefone}@c.us`, mensagem).then(() => {
                        console.log(chalk.green(`[✔] Mensagem de casamento enviada para ${nome} (${numeroTelefone})`));
                    }).catch((error) => {
                        console.log(chalk.red(`[X] Erro ao enviar mensagem de casamento para ${nome} (${numeroTelefone})`));
                        console.log(error);
                    });
            ...
            console.log(chalk.red('[X] Falha ao obter dados da API de aniversários (casamento). Contate o programador!'));
            console.log(aniversarios.mensagem);
        }
    } catch (error) {
        console.log(chalk.red('[X] Erro ao fazer requisição para a API de aniversários (casamento):', error));
    }
};
...
client.on('ready', () => {
    ...
    setInterval(buscarMensagensCasamentos, 86400000); // 86400000ms = 24 horas
});
